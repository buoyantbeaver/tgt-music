...
//$cache = new cache();
//if ( $cache->caching ) {
$song 		= $tgtdb->databasetgt(" m_title, m_singer, m_cat, m_img, m_url, m_viewed, m_lyric, m_kbs, m_sang_tac,  m_lyricLRCNCT, m_lyricLRC, m_is_local ","data"," m_id = '".$id_media."' ORDER BY m_id DESC ");
...
$song_url_jwplayer = url_link($song[0][0].'-'.$title,$id_media,'nghe-bai-hat');
$song_url_directLink = grab(get_url($song[0][11],$song[0][4]));
//Lay anh cua bai hat
$song_image_directLink = $song[0][3];
$song_lyric_LRC = $song[0][10];
?>
...
<?}?>
<!-- nghe ca khuc bang jwplayer 7.x -->
			<h4 align="center"><a href="<? echo $song_url;?>" title="">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng JWPlayer 7.x</a></h4>
                    </div>
					<?=BANNER('play_mp3','633');?>
...
<div class="player">
	...
	<script type="text/javascript">
		var ap3 = new APlayer({
			...
			mode: 'circulation',
			music: {
				title: '<? echo un_htmlchars($song[0][0]);?>',
				author: '<? echo un_htmlchars($title);?>',
				url: '<? echo $song_url_directLink;?>',
				pic: '<? echo $song_image_directLink;?>',
				lrc: "<? echo $song_lyric_LRC;?>"
			}
		});
	</script>
</div>
...
<? if($singer_info) { ?>
            <div class="box w_4">
            	<h1>Thông tin ca sĩ <?=$title ;?><a onclick="SINGERHIDESHOW();" id="singer_txt"> Ẩn thông tin</a></h1>
                <div id="singer_info" class="singer_info">
