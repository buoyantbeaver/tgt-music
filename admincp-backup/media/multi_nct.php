...
/* Code AG-MUSIC 2014 */
/* By Achieve Gemini */
/* Email: quocdat96.it@gmail.com */

function find_value($str,$find_start,$find_end)
{
          ...
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td class="title_c">THÊM ALBUM TỪ NCT 01/01/2014</td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
...
<tr><td class=title align=center>THÊM ALBUM NHACCUATUI BY AG MUSIC 01/01/2014</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ DẠNG LINK POST: http://www.nhaccuatui.com/playlist/v-rap-greatest-hits-022013-va.Mf7R7QqrKk17.html<br>
</td>
</tr>
...
if (!$_POST['submit']) {
    	$url = getPage($total_links,'','30','');
	$url_song = explode('<div class="item_content">', $url);
	$total_linksx = count($url_song);
    	$total_links = $total_linksx-1;
	$album_z = explode('<meta property="og:title" content="', $url);
	$album_z = explode(' - ', $album_z[1]);
	$singer_z = explode('"/>', $album_z[1]);
    	$album_z = $album_z[0];
    	$singer_z = $singer_z[0];
	$album_info = explode('<p id="shortPlDesc">', $url);
	$album_info = explode('...&nbsp;', $album_info[1]);
	$album_info = str_replace('NhacCuaTui.Com',"Nhac24 Music",$album_info[0]);
	$album_img  = explode('<meta property="og:image" content="', $url);
...
<tr>
   ...
   <tr> <td><font color="grenn">CA SĨ:</font></td><td><input name="new_singer_a" value="<?=$singer_z?>" size=50></td></tr>
   </table>
	</td>
</tr>
...
for ($i=1;$i<=$total_links;$i++) {
	...
	$play_url = explode('" onclick="NCTNowPlaying.playSong', $play_url[1]);
    $play_url = $play_url[0];
...
?>
<tr>
<td class=fr width='30%' align="right">TÊN BÀI HÁT <?=$i?></td>
...
<?php
	...
	echo "Ok<meta http-equiv='refresh' content='0;url=media.php?mode=multi_nct'>";
}
}
?>
                </td>
