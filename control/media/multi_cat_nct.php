...
<tr><td class=title align=center>Thêm Media từ Thể loại NCT</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ Định dạng : http://www.nhaccuatui.com/tim_kiem/bai_hat?g=nhactre<br>
</td>
</tr>
...
if (!$_POST['submit']) {
	...
	$url_song = explode('" class="imusic">', $url);
	$total_linksx = count($url_song);
    $total_links = $total_linksx-1;
	$name_cat = explode('<h1 class="title">', $url);
	$name_cat = explode('</h1>', $name_cat[1]);
?>
...
for ($i=1;$i<=$total_links;$i++) {
	$play_url = explode('"/nghe?M=', $url_song[$i]);
	$play_url = explode('"', $play_url[1]);
    $txt2='http://www.nhaccuatui.com/nghe?M='.$play_url[0];
    $song = explode('">', $url_song[$i]);
    $song = explode('</a>', $song[1]);
    $song = $song[0];
	$singer = explode('b=singer" >', $url_song[$i]);
...
<?php
	...
	echo "<h3>Đã update thành công</h3><meta http-equiv='refresh' content='3;url=$link'>";
}
}
?>
                </td>
