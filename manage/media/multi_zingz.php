          ...
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td class="title_c">Thêm Media từ Zing Channel</td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
...
<tr><td class=title align=center>Thêm Media từ Zing Channel</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ Định dạng : http://mp3.zing.vn/the-loai-video/Nhac-Tre/IWZ9Z088.html?sort=total_play&p=xx<br>
</td>
</tr>
...
if (!$_POST['submit']) {
	...
	$url_song = explode('<div class="content-item ', $url);
...
<tr>
	<td class=fr width=30% align="right">
		<font color=red><strong>Ca sỹ</strong></font></td>
	<td class=fr_2>
		<!--<input size=50 onkeyup="new_singer(this.value)"><br />-->
		<select onChange="new_stype(this.value)">
       		<option value=1>Ca sỹ VN</option>
        	<option value=2>Ca sỹ AM</option>
       		<option value=3>Ca sỹ CA</option>
		</select>
	</td>
</tr>
...
for ($i=1;$i<=$total_links;$i++) {
    ...
    $song = explode('class="new-video-img" title="', $url_song[$i]);
    $song = explode(' - ', $song[1]);
    $song = $song[0];
	$singer = explode('<a class="_strCut" strlength="20" title="', $url_song[$i]);
    $singer = explode('"', $singer[1]);
    $singer = $singer[0];
	$images = explode('src="http://image.mp3.zdn.vn/', $url_song[$i]);
    $images = explode('" />', $images[1]);
    $images = 'http://image.mp3.zdn.vn/'.$images[0];
?>
<tr>
...
<td class=fr align="right">Ca sỹ</td>
...
<tr>
<td class=fr width='30%' align="right">Ảnh<?=$i?></td>
