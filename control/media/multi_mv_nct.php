          ...
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td class="title_c">Thêm Media từ NCT</td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
...
<tr><td class=title align=center>Thêm Media từ NCT</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ �?nh d?ng : http://www.nhaccuatui.com/mv/viet-nam.html<br>
</td>
</tr>
...
if (!$_POST['submit']) {
	...
	$url_song = explode(' <a href="/mv', $url);
...
for ($i=1;$i<=$total_links;$i++) {
	$play_url = explode('href="/mv', $url_song[$i]);
	$play_url = explode('"', $play_url[1]);
    $txt2='http://www.nhaccuatui.com/mv'.$play_url[0];
    $song = explode('" title="', $url_song[$i]);
    $song = explode(' - ', $song[1]);
    $song = $song[0];
	$singer = explode('singer" >', $url_song[$i]);
    $singer = explode('</a>', $singer[1]);
    $singer = $singer[0];
	$images = explode('<img src="', $url_song[$i]);
    $images = explode('" alt="', $images[1]);
    $images = $images[0];
?>
<tr>
<td class=fr width='30%' align="right">T�n Media <?=$i?></td>
...
<?php
	...
	echo "Th�nh c�ng<meta http-equiv='refresh' content='0;url=$link'>";
}
}
?>
                </td>
