...
if (!$_POST['submit']) {
    ...
    <table cellspacing="2" cellpadding="2" width="100%" class=border>
        ...
        <tr><td class=fr>Tài khoản FTP <?=$i?>:</td><td class=fr_2><input name="local_user_[<?=$r['local_id']?>]" size="50" value="<?=$r['local_user']?>"></td></tr>
        <tr><td class=fr>Mật khẩu FTP <?=$i?>:</td><td class=fr_2><input name="local_pass_[<?=$r['local_id']?>]" size="50" value="<?=$r['local_pass']?>"></td></tr>

		<? $i++; } ?>
        <tr><td align="center" class=fr_2>Tổng Số Server <input size=4 name="local_total" value="<?=$total?>" onClick="this.select()"></td><td class=fr><input type="submit" value="ĐỒNG Ý" name=submit class=submit></td></tr>
        <tr><td colspan=2>
        </td></tr>
    </td></tr>
    </table>
