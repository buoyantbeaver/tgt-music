...
//$cache = new cache();
//if ( $cache->caching ) {
$song 		= $tgtdb->databasetgt(" m_title, m_singer, m_cat, m_img, m_url, m_viewed, m_lyric, m_kbs, m_sang_tac,  m_lyricLRCNCT, m_lyricLRC, m_lyricKAR ","data"," m_id = '".$id_media."' ORDER BY m_id DESC ");
...
$As3karaokeSongInfo = str_replace("'", " ", un_htmlchars($song[0][0])).','.$lyricKAR;
//substr(SITE_LINK,0,-1): cắt 1 ký tự bên phải
$lyricKAR = str_replace("'", " ", un_htmlchars($song[0][11]));
$As3karaokeSongInfo = str_replace("'", " ", un_htmlchars($song[0][0])).','.$lyricKAR;
//Liệt kê các player
$lyricZSTAR = str_replace("'", " ", un_htmlchars($song[0][9]));
$song_url_karaoke_jw7 = url_link($song[0][0].'-'.$title,$id_media,'nghe-bai-hat');
$song_url_karaoke_zmp3 = url_link($song[0][0].'-'.$title,$id_media,'nghe-bai-hat-zmp3');
$song_url_karaoke_lrc = url_link($song[0][0].'-'.$title,$id_media,'nghe-bai-hat-lrc');
$song_url_karaoke_nct = url_link($song[0][0].'-'.$title,$id_media,'nghe-bai-hat-nct');
...
<h1><? echo un_htmlchars($song[0][0]);?> - <span class="singer_"><a href="<? echo $singer_url;?>" title="Bài hát của ca sĩ <? echo $title;?>"><? echo un_htmlchars($title);?></a></span></h1>
			...
			<!--nghe ca khuc bang as3 karaoke player HTML " thay bằng &#34;-->
			<h4 align="center"><a href="<? echo $song_url_karaoke_lrc;?>" title="nghe ca khúc: &#34;<?=un_htmlchars($song[0][0]);?>&#34; bằng APlayer HTML5 Karaoke">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng APlayer HTML5 Karaoke</a></h4>
			<h4 align="center"><a href="<? echo $song_url_karaoke_jw7;?>" title="nghe ca khúc: &#34;<?=un_htmlchars($song[0][0]);?>&#34; bằng JWPlayer 7.x Player">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng JWPlayer 7.x Player</a></h4>
			<h4 align="center"><a href="<? echo $song_url_karaoke_zmp3;?>" title="nghe ca khúc: &#34;<?=un_htmlchars($song[0][0]);?>&#34; bằng ZingMP3 Karaoke Flash Player">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng Zing MP3 Karaoke Flash Player 6.x</a></h4>
			<h4 align="center"><a href="<? echo $song_url_karaoke_nct;?>" title="nghe ca khúc: &#34;<?=un_htmlchars($song[0][0]);?>&#34; bằng NCT Karaoke Flash Player">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng NCT Karaoke Flash Player</a></h4>
<?if(strlen($lyricKAR)>5){?>
			<h4 align="center"><a href="<? echo $song_url_karaoke_as3;?>" title="nghe ca khúc: &#34;<?=un_htmlchars($song[0][0]);?>&#34; bằng AS3 Flash Karaoke Player">nghe ca khúc: "<? echo un_htmlchars($song[0][0]);?>" bằng AS3 Flash Karaoke</a></h4>
<?}?>
...
<div class="player">
<object id="player" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="100%" height="411">
	<param name="movie" value="<? echo SITE_LINK; ?>includes/skins/zingStarPlayer.swf" />
	<param name="quality" value="high" />
	<param name="wmode" value="transparent" />
	<param name="allowscriptaccess" value="always"/>

	<param name="allowfullscreen" value="true"/>
	<param name="flashvars" value="status=karaoke&song_id=<? echo en_id($id_media);?>&domain=<? echo substr(SITE_LINK,0,-1); ?>&austart=true">
	<embed id="player" wmode="transparent" allowscriptaccess="always" allowfullscreen="true" width="100%" height="411" src="<? echo SITE_LINK; ?>includes/skins/zingStarPlayer.swf" flashvars="status=karaoke&song_id=<? echo en_id($id_media);?>&domain=<? echo substr(SITE_LINK,0,-1); ?>&austart=true" />
</object>
</div>
            <!-- begin shared -->
