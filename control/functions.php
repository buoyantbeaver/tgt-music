...
function acp_type($i) {
	$html = "<select name=\"type\" class=\"form-control findasong\">".
		"<option value=1".(($i==1)?' selected':'').">AUDIO</option>".
...
function acp_cat($id = 0, $add = false) {
	...
	echo '<div class="findasong" style="margin-right:10px;"><strong>Danh sách thể loại</strong><br><select class="form-control" style="margin-bottom:5px;" multiple name="fromBoxCat" id="fromBoxCat">';
	...
	////
	echo "</select><span><a class=\"btn btn-xs btn-default\" href=\"#\" id=\"add\">Thêm thể loại »</a></span></div>";
	echo "<div class=\"findasong\"><strong>Thể loại đã chọn</strong><br><select class=\"form-control\" style=\"margin-bottom:5px\" multiple name=\"cat[]\" id=\"cat\">";
	$cat_all	=	substr($id, 1);
	...
	echo "</select>";
	echo "<span><a class=\"btn btn-xs btn-default\" href=\"#\" id=\"remove\">« Xóa thể loại</a></span></div>";
}
...
function acp_server($id) {
	...
	$html = "<select name=\"server\" class=\"form-control findasong\">";
	$html .= "<option value=0>---Không chọn---</option>";
	for($z=0;$z<count($arr);$z++) {
...
function acp_singer($id = 0, $add = false) {
	...
	$html = "<select name=singer class=\"form-control findasong\">";
...
function acp_singer_type($i) {
	$html = "<select name=\"singer_type\" class=\"form-control findasong\">".
...
function them_moi_singer_form() {
	$html = "<input name=new_singer class=\"singer_1 form-control findasong\" size=35>  <select class=\"singer_2 form-control findasong\" name=singer_type>".
		"<option value=1".(($i==1)?' selected':'').">CA SĨ VIỆT NAM</option>".
		"<option value=2".(($i==2)?' selected':'').">CA SĨ ÂU MỸ</option>".
		"<option value=3".(($i==3)?' selected':'').">CA SĨ CHÂU Á</option>".
	"</select>";
...
if (preg_match("#youtube.com/v/([^/-]+)#s",$url,$id_yt)){
		$img = 'http://img.youtube.com/vi/'.$id_yt[1].'/1.jpg';
	}
elseif (preg_match("#youtube.com/watch\?v=([^/]+)#s",$url, $id_sr)){
		$img = 'http://img.youtube.com/vi/'.$id_sr[1].'/1.jpg';
	}
