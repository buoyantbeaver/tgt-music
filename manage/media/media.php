...
if($mode == 'add') {
	if(isset($_POST['submit'])) {
		...
		if($_POST['song'] && $_POST['url']) { 	
			$song		 = htmlchars(stripslashes(trim($_POST['song'])));
			//$song	=	htmlspecialchars($song,ENT_COMPAT);
			//$song	=	htmlspecialchars($song,ENT_QUOTES);
			//$song	=	htmlspecialchars($song,ENT_NOQUOTES);
			...
			$type		 = $_POST['type'];
			if(move_uploaded_file ($_FILES['img']['tmp_name'],FOLDER_VIDEO."/[TGT-music]-".time()."-".$_FILES['img']['name'])) {
					$img = LINK_VIDEO."/[TGT-music]-".time()."-".$_FILES['img']['name'];
			}
			elseif($_POST['grab_img']) $img = grab_img($url);
			else $img = $_POST['img'];
			
			$action		 = "media.php?mode=add";
			mysql_query("INSERT INTO tgt_nhac_data (m_title,m_title_ascii,m_singer,m_cat,m_poster,m_sang_tac,m_lyric,m_lyricLRCNCT,m_lyricCaption,m_lyricSRT,m_lyricLRC,m_lyricKAR,m_type,m_url,m_img,m_is_local,m_time) 
						 VALUES ('".$song."','".$song_ascii."','".$singer."','".$cat."','".$user_id."','".$sangtac."','".$lyric."','".$lyricLRCNCT."','".$lyricCaption."','".$lyricSRT."','".$lyricLRC."','".$lyricKAR."','".$type."','".$url."','".$image."','".$local."','".NOW."')");
...
if($mode == 'edit') {
		...
		$id	= del_id($id);
		$arrz = $tgtdb->databasetgt(" m_id, m_title, m_singer, m_cat, m_url, m_img, m_is_local, m_lyric, m_type, m_sang_tac, m_viewed, m_viewed_month, m_lyricLRCNCT, m_lyricCaption, m_lyricSRT, m_lyricLRC, m_lyricKAR  ","data"," m_id = '$id'");
		...
		if(isset($_POST['submit'])) {
			if($_POST['song'] == "" || $_POST['url'] == "") {
				...
				$lyricCaption		 = htmlchars(stripslashes(trim(urldecode($_POST['lyricCaption']))));
                                $image  	 = htmlchars(stripslashes(trim(urldecode($_POST['image']))));
				$type		 = $_POST['type'];
				if(move_uploaded_file($_FILES['img']['tmp_name'],FOLDER_VIDEO."/[KM-Music]-".time()."-".$_FILES['img']['name'])) {
					delFile($arrz[0][5]);
					$img = LINK_VIDEO."/[KM-Music]-".time()."-".$_FILES['img']['name'];
				}
				elseif($_POST['grab_img']) $img = grab_img($url);
				else $img = $arrz[0][5];
...
if($mode == 'multi_zingz_tv') {
	include("multi_zingz_tv.php");
}
if($mode == 'multi_zingz_singer') {
	include("multi_zingz_singer.php");
}
if($mode == 'multi_zing_song_cat') {
	include("multi_zing_song_cat.php");
}
if($mode == 'multi_zing_song_singer') {
	include("multi_zing_song_singer.php");
}
