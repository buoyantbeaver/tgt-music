<?php
	// phan trang
	$sql_tt = "SELECT album_id  FROM tgt_nhac_album LEFT JOIN tgt_nhac_singer ON (tgt_nhac_album.album_singer = tgt_nhac_singer.singer_id) WHERE tgt_nhac_singer.singer_type = '1' AND album_type = '1' ORDER BY album_id DESC LIMIT ".LIMITSONG;

	$rStar = HOME_PER_PAGE * ($page -1 );
	$arr_album = $tgtdb->databasetgt(" album_id, album_name, album_singer, album_viewed, album_img, album_type, album_cat, album_poster, album_time, album_song ","album  LEFT JOIN tgt_nhac_singer ON (tgt_nhac_album.album_singer = tgt_nhac_singer.singer_id)"," tgt_nhac_singer.singer_type = '1' AND album_type = '1'  ORDER BY album_id DESC LIMIT ".$rStar .",". HOME_PER_PAGE,"");
	$cat_name = get_data("theloai","cat_name"," cat_id = '".$id."'");
	$phantrang = linkPage($sql_tt,HOME_PER_PAGE,$page,"Album/Viet-Nam/#page#","");
?>
                <!-- begin song -->
        	<div class="box w_2">
            	<h1>Danh sách Playlist thành viên</h1>
                <div class="padding">
					<div>
<? 
...
?>
                    ...
                    <div class="playlist_list border_bottom">
                    <table width="100%" border="0" cellpadding="2" cellspacing="2">
                        <tr>
                            ...
                            <td valign="top" align="left" class="fjx_padding">
                                <table width="100%"  border="0" cellpadding="2" cellspacing="2">
                                    <tr><td colspan="2" class="album_title"><h4><span class="singer_"><a title="Nghe Playlist <? echo $arr_album[$i][1]; ?>" href="<? echo $album_url; ?>"><? echo un_htmlchars($arr_album[$i][1]);?></a></span> - <span class="singer_"><a href="<? echo $singer_url;?>" title="Bài hát của ca sĩ <? echo un_htmlchars($singer_name);?>"><? echo un_htmlchars($singer_name);?></a></span></h4></td></tr>
									...
									<tr><td>Đăng bởi: </td><td><? echo get_user($arr_album[$i][7],'user');?></td></tr>
                                    <tr></tr>
                                </table>
                            </td>
                        </tr>
