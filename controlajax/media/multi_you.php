...
<tr><td class=title align=center>Add video off Youtube</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ Link post : http://www.youtube.com/results?search_query=Dan+Truong<br>
</td>
</tr>
...
if (!$_POST['submit']) {
	...
	$url_song = explode('<h3 class="yt-lockup-ellipsize">', $url);
...
for ($i=1;$i<=$total_links;$i++) {
    ...
    $song = explode('translation-title"dir="ltr"title="', $url_song[$i]);
    $song = explode('"data-sessionlink', $song[1]);
    $song = $song[0];
	$singer = explode('translation-title"dir="ltr"title="', $url_song[$i]);
    $singer = explode('"data-sessionlink', $singer[1]);
    $singer = $singer[0];
	$images = explode('href="/watch?v=', $url_song[$i]);
    $images = explode('">', $images[1]);
    $images = 'http://i2.ytimg.com/vi/'.$images[0].'/default.jpg';
?>
<tr>
