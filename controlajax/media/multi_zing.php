          ...
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td class="title_c">THÊM ALBUM ZING MP3 BY KM MUSIC 26/12/2012</td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
...
<tr><td class=title align=center>THÊM ALBUM ZING MP3 BY KM MUSIC 26/12/2012</td></tr>
<tr>
	<td class=fr width=10% align=center>
	<input name="total_songs" size=100 value="" style="text-align:center">
	<br><br>
+ Link post: http://mp3.zing.vn/album/Cuoc-Tinh-Da-Mat-Dam-Vinh-Hung/ZWZ977EC.html<br>
</td>
</tr>
...
if (!$_POST['submit']) {
	...
	$url_song = explode('<li id="_plItem', $url);
	$total_linksx = count($url_song);
    $total_links = $total_linksx-1;
	$album_z = explode('<meta name="title" content="', $url);
	$album_z = explode(' - ', $album_z[1]);
	$singer_z = explode(' |', $album_z[1]);
    $album_z = $album_z[0];
    $singer_z = $singer_z[0];
	$album_info = explode('class="rows2" expand="rows2">', $url);
	$album_info = explode('</p>', $album_info[1]);
	$album_info = str_replace('Zing Mp3',"KM Music",$album_info[0]);
...
<tr>
    ...
    <tr><td><font color="blue">DANH MỤC ALBUM</font></td><td>
    	<select name="singer_type_a" onChange="new_stype(this.value)">
			...
			<option value=4>ALBUM DJ</option>
		</select></td></tr>
   <tr> <td><font color="grenn">CA SĨ:</font></td><td><input name="new_singer_a" value="<?=$singer_z?>" size=50></td>
    </tr></table>
	</td>
</tr>
...
for ($i=1;$i<=$total_links;$i++) {
	$play_url = explode('href="/bai-hat/', $url_song[$i]);
	$play_url = explode('"', $play_url[1]);
    $play_url = 'http://mp3.zing.vn/bai-hat/'.$play_url[0];
    $song = explode('sname="', $url_song[$i]);
    $song = explode('"', $song[1]);
    $song = $song[0];
	$singer = explode('aname="', $url_song[$i]);
    $singer = explode('"', $singer[1]);
    $singer = $singer[0];
?>
<tr>
<td class=fr width='30%' align="right">TÊN BH <?=$i?></td>
...
<?php
	...
	echo "Ok<meta http-equiv='refresh' content='0;url=media.php?mode=multi_zing'>";
}
}
?>
                </td>
