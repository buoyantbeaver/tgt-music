	...
	if($pages > 1){
		...
		if($curPage > 1){
			$prePage = $curPage - 1;
				...
				$strlink .= "<li><a href=\"".$tempurl.".html\"><</a></li>";
		}
		...
		for($i=$i; $i<=$ubound;$i++){
			if($i == $curPage)
				$strlink .= "<li class=\"active\"><a>".$i . "</a></li>";
					...
					$strlink .= "<li><a href=\"".$tempurl.".html\">".$i."</a></li>";
			}
		}// for

		if($totalRecord%$perPage != 0)
			if($i == $curPage)
				$strlink .= "<li class=\"active\"><a>".$i . "</a>";
			else{
					$tempurl = str_replace("#page#",$i, $url);
					$strlink .= "<li><a href=\"".$tempurl.".html\">".$i."</a></li>";
			}
		...
		if($all != "") {
				...
				$strlink .= "<li><a href=\"".$tempurl.".html\"'>view all</a></li>";
			}
...
function url_link($name,$id,$type,$x=0) {
	...
	$name 	= replace($name);
		switch($type) {
			...
			case 'the-loai'			: $url = SITE_LINK.'nhac/'.$id.".html"; break;
			...
			case 'nghe-bai-hat-nct'	: $url = SITE_LINK.'bai-hat-nct/'.$name.'/'.$id.".html"; break;
			case 'nghe-bai-hat-lrc'	: $url = SITE_LINK.'bai-hat-lrc/'.$name.'/'.$id.".html"; break;
			case 'nghe-bai-hat-kar'	: $url = SITE_LINK.'bai-hat-kar/'.$name.'/'.$id.".html"; break;
                        case 'nghe-bai-hat-zstar'	: $url = SITE_LINK.'bai-hat-zstar/'.$name.'/'.$id.".html"; break;
			case 'singer-cat'		: $url = SITE_LINK.'the-loai-ca-sy/'.$name.'/'.$id.".html"; break;			
			case 'xem-video'		: $url = SITE_LINK.'video/'.$name.'/'.$id.".html"; break;
			case 'nghe-album'		: $url = SITE_LINK.'playlist/'.$name.'/'.$id.'-'.$x.".html"; break;
			case 'nghe-album-lrc'	: $url = SITE_LINK.'playlist-lrc/'.$name.'/'.$id.'-'.$x.".html"; break;
			case 'nghe-album-kar'	: $url = SITE_LINK.'playlist-kar/'.$name.'/'.$id.'-'.$x.".html"; break;
                        case 'nghe-album-bt1'	: $url = SITE_LINK.'playlist-bt1/'.$id.".html"; break;
			case 'user'		        : $url = SITE_LINK.'Member/Z/'.$name.'/'.del_id($id).".html"; break;
			case 'item'		        : $url = SITE_LINK.'news/'.$name.'/'.$id.".html"; break;
		}
return $url;
}
...
$url = SITE_LINK.'news/'.$name.'/'.en_id($id).".html";
...
function check_type($type,$id_img){
	...
	$media_type = "<i class=\"fa fa-film\" aria-hidden=\"true\"></i>";
	}
	else {
	$media_type = "<i class=\"fa fa-music\" aria-hidden=\"true\"></i>";
	}
	return $media_type;
}
...
//cac ham phuc vu lay link mp3 bai-hat nct
function getArrayXml($link){
    $content = curl($link);
...
function getMp3($link){
    $xml_arr = getArrayXml($link);
    $return = array();
	$return_link_mp3 = "";
    if($xml_arr['track']){
		$item = $xml_arr['track'];
        $return_link_mp3   = $item['location'];
    }
    return $return_link_mp3;
}
function curl($url)
{
	$ch = @curl_init();
...
//end of kiem tra lay link mp3 bai-hat nct
function box_online() {
	$sid = session_id();
    $CurrentTime = time();
