          ...
          <td valign="top" class="style_bg"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td class="title_c">Danh sách &gt; Tin Tức</td>
                    <td align="right" valign="middle" class="title_c">
                   ID news cần sửa: 
                   <input id=m_id size=10 value="" class="input">
                      <input type="image" src="../images/b_search.gif" onclick='window.location.href = "news.php?mode=edit&id="+document.getElementById("m_id").value;'>
                    
                    ID ca sĩ cần xóa: 
                    <input id=m_del_id size=10 value="" class="input">
 <input type="image" src="../images/b_search.gif" onclick='window.location.href = "news.php?del_id="+document.getElementById("m_del_id").value;'>
                    
                    Tìm theo tên ca sĩ: <input id=search size=40 value="<? echo $search;?>" class="input">
                      <input type="image" src="../images/b_search.gif" onclick='window.location.href = "list_news.php?search="+document.getElementById("search").value;'>&nbsp;&nbsp;&nbsp; </td>
                  </tr>
                </table>               </td>
              </tr>
            </table>
		    ...
		    <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td colspan="">
                <form name=media_list method=post action='list_news.php' onSubmit="return check_checkbox();">
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr >
                    ...
                    <td nowrap class="menu">hình ảnh </td>
                      <td nowrap class="menu" width="80%">Tên album </td>
                      <td width="10%" align="center" nowrap class="menu">Chức năng</td>					  
                    </tr>
<?
	$arrz = $tgtdb->databasetgt(" news_id, news_title, news_title_ascii, news_img  ","news"," $sql_where $sql_order LIMIT ".$rStar .",". HOME_PER_PAGE,"");
	for($i=0;$i<count($arrz);$i++) {

?>
<tr onMouseOver="bgColor='#c9daf6'" onMouseOut="bgColor='#FFFFFF'">
						  <td nowrap align="center"><input class=checkbox type=checkbox id=checkbox onclick=docheckone() name=checkbox[] value=<? echo $arr_news[$z][0];?>></td>
                       <td align="center" nowrap ><img class=IMG_S src="<? echo check_img2($arrz[$i][3]);?>"></td>	
					  <td class="song_name" align="left" style="padding-left: 10px;" nowrap  ><a href="news.php?mode=edit&id=<? echo $arrz[$i][0] ?>" title="Edit this industry"><? echo $arrz[$i][1];?></a><br> </td>						
				    <td align="center" nowrap  style="padding-left:7px"><a href="news.php?mode=edit&id=<? echo $arrz[$i][0];?>" title="Edit this industry"><img src="../images/edit.png" width="16" height="16" border="0"></a> 
						  <a href="news.php?del_id=<? echo $arrz[$i][0];?>" title="Xóa danh mục này" ><img src="../images/b_delete.png" width="11" height="14" border="0"></a></td>
						</tr>
...
if ($_POST['do']) {
	...
	if ($_POST['selected_option'] == 'del') {
		...
		if (is_numeric($posi) || is_numeric($posi2)) mss ("Trong danh sách có ca sĩ không thể xóa ","list_news.php");
		else {
		mysql_query("DELETE FROM tgt_nhac_news WHERE news_id IN (".$in_sql.")");
		mss ("Xóa thành công ","list_news.php");
