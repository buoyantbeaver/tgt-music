<?php
	// phan trang
	$sql_tt = "SELECT album_id  FROM tgt_nhac_album LEFT JOIN tgt_nhac_singer ON (tgt_nhac_album.album_singer = tgt_nhac_singer.singer_id) WHERE tgt_nhac_singer.singer_type = '3' AND album_type = '0' ORDER BY album_id DESC LIMIT ".LIMITSONG;

	$rStar = HOME_PER_PAGE * ($page -1 );
	$arr_album = $tgtdb->databasetgt(" album_id, album_name, album_singer, album_viewed, album_img, album_type, album_cat, album_poster, album_time, album_song ","album  LEFT JOIN tgt_nhac_singer ON (tgt_nhac_album.album_singer = tgt_nhac_singer.singer_id)"," tgt_nhac_singer.singer_type = '3' AND album_type = '0'  ORDER BY album_id DESC LIMIT ".$rStar .",". HOME_PER_PAGE,"");
	$cat_name = get_data("theloai","cat_name"," cat_id = '".$id."'");
	$phantrang = linkPage($sql_tt,HOME_PER_PAGE,$page,"Album/Viet-Nam/#page#","");
?>
                <!-- begin song -->
        	<div class="box w_2">
            	<h1>Danh Sách Album Châu Á</h1>
                <div class="padding">
					<div>
<? 
