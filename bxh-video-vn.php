...
$up = $tgtdb->databasetgt(" * ","cf"," cf_id = 4");
?>
...
<title>Bảng Xếp Hạng Video Việt Nam Tháng <? echo date('m');?></title>
<meta name="title" content="Bảng xếp hạng video Việt Nam tháng <? echo date('m');?>" />
<meta name="keywords" content="Bảng xếp hạng video Việt Nam tháng <? echo date('m');?>" />
<meta name="description" content="Bảng xếp hạng video Việt Nam tháng <? echo date('m');?>" />
...
<div id="main">
 ...
 <div class="bxh_title v_vn_ _bottom"><span>BXH</span> Video Việt Nam<a href="BXH/Video/Viet-Nam.html?act=play"><img class="right" src="temp/img/play.gif" border="0" /></a></div>
 ...
 <div class="_player">
	...
	<param name="flashvars" value="file=<? echo SITE_LINK; ?>xml/6/bxh-video-vn.xml&playlistsize=0&playlist=bottom&autostart=true&songid=0&uservip=true&zoomsizeon=false&ads=">
	<embed id="player" wmode="transparent" allowscriptaccess="always" allowfullscreen="true" width="100%" height="400" src="<? echo SITE_LINK; ?>tgt_file/player.swf" flashvars="file=<? echo SITE_LINK; ?>xml/6/bxh-video-vn.xml&playlistsize=0&playlist=bottom&autostart=true&songid=0&zoomsizeon=false&ads=" />
</object>
 </div>
 <? } ?>
 <div class="bxh_list _delline">
                <?
				$arr_vn_ = $tgtdb->databasetgt(" m_id, m_title, m_singer, m_img ","data"," m_cat LIKE '%,".IDCATVN.",%' AND m_type = 2 ORDER BY m_viewed_month DESC LIMIT 40");				
				for($i=0;$i<count($arr_vn_);$i++) {
					$list_song	.=	$arr_vn_[$i][0].',';
					$list_song_ = 	substr($list_song,0,-1);
				if(date("w") == 0 && $up[0][4] == '1') {
					mysql_query("UPDATE tgt_nhac_cf SET cf_value = '".$list_song_."', cf_date = '".date("d/m/Y")." - ".tinh_tuan(7)."', cf_up = '0' WHERE cf_id = '4'");
				}
