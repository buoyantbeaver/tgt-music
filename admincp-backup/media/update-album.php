...
elseif ($_POST['xSongAlbum']){
	...
	foreach($s as $x=>$val) {
		...
		$stt	=	$x+1;
		echo '<li id="arrayorder_'.$arr[$x][0][0].'"><div class="left">'.$stt.'. '.$arr[$x][0][1].' - '.$singer_name.'</div><div class="right"><a onClick="xSongAlbum('.$album_id.','.$arr[$x][0][0].');" style="cursor: pointer;">X</a></div><div class="clr"></div></li>';
	}
}
?>
